...
NoShowMessageReturn = function(msgType, isSelfPlayer)
              ...
              if messageType.fitnessLevelUp == msgType then
                ...
                if messageType.territoryWar_Start == msgType or messageType.territoryWar_End == msgType or messageType.territoryWar_Add == msgType or messageType.territoryWar_Destroy == msgType or messageType.territoryWar_Attack == msgType or messageType.inSiegeArea == msgType or messageType.outSiegeArea == msgType then
                  ...
                  if messageType.guildWar_Start == msgType or messageType.guildWar_End == msgType then
                    ...
                    if messageType.roulette == msgType or messageType.anotherPlayerGotItem == msgType or messageType.enchantFail == msgType or messageType.horseNineTier == msgType or messageType.enchantSuccess == msgType then
                      msgTypeNum = 6
                    else
                      if messageType.itemMarket == msgType then
                        msgTypeNum = 7
...
Proc_ShowMessage_Ack_For_RewardSelect = function(message, showRate, msgType, exposure, isSelfPlayer)
...
Proc_ShowMessage_Ack_WithOut_ChattingMessage_For_RewardSelect = function(message, showRate, msgType, exposure, isSelfPlayer)
  ...
  if ToClient_getJoinGuildBattle() == true and (exposure == nil or exposure == true) then
    return 
  end
  if ToClient_IsGuildTeamBattleIng() == true and ToClient_IsSelfPlayerInGuildTeamBattleTerritory() == true and (exposure == nil or exposure == true) then
    return 
  end
...
FromClient_NotifyVariableTradeItemMsg = function(territoryKey, npcCharacterKey, eventIndex)
  -- function num : 0_6 , upvalues : _npcKey, npcKey, _commerceType, _tradeMsgType, tradeEvent
  local temporaryWrapper = getTemporaryInformationWrapper()
    ...
    for ii = 1, enCommerceType.enCommerceType_Max - 1 do
      if commerceMsg == "" then
        ...
        if varyTradeInfo:getPercentByCommerceType(ii) > 0 then
          commerceMsg = _commerceType[ii] .. " <PAColor0xFFFFCE22>â–\178" .. varyTradeInfo:getPercentByCommerceType(ii) / 10000 .. "<PAOldColor>%"
          ...
          if varyTradeInfo:getPercentByCommerceType(ii) < 0 then
            ...
            commerceMsg = _commerceType[ii] .. " <PAColor0xFFF26A6A>â–\188" .. varyTradeInfo:getPercentByCommerceType(ii) / 10000 .. "<PAOldColor>%"
        ...
        if varyTradeInfo:getPercentByCommerceType(ii) > 0 then
          commerceMsg = commerceMsg .. " | " .. _commerceType[ii] .. " <PAColor0xFFFFCE22>â–\178" .. varyTradeInfo:getPercentByCommerceType(ii) / 10000 .. "<PAOldColor>%"
          ...
          if varyTradeInfo:getPercentByCommerceType(ii) < 0 then
            commerceMsg = commerceMsg .. " | " .. _commerceType[ii] .. " <PAColor0xFFF26A6A>â–\188" .. varyTradeInfo:getPercentByCommerceType(ii) / 10000 .. "<PAOldColor>%"
                                                                ...
                                                                do
                                                                  ...
                                                                  do
                                                                    ...
                                                                    if processIndex < curIndex and tempMsg == ((MessageData._Msg)[processIndex + 1]).msg then
                                                                      ...
                                                                      ;
                                                                      ((MessageData._Msg)[processIndex]).msg = nil
                                                                      -- DECOMPILER ERROR at PC2192: Confused about usage of register: R1 in 'UnsetPending'
