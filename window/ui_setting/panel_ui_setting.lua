...
function UiSet_ConfrimSetting_Sub(isReset)
  ...
  for idx = 1, UiSet.panelCount do
    ...
    elseif idx == panelID.ClassResource then
      if false == _ContentsGroup_RenewUI_Main and (CppEnums.ClassType.ClassType_Sorcerer == getSelfPlayer():getClassType() or CppEnums.ClassType.ClassType_Combattant == getSelfPlayer():getClassType() or CppEnums.ClassType.ClassType_CombattantWomen == getSelfPlayer():getClassType()) then
        panelControl[idx].control:SetShow(panelControl[idx].isShow)
      end
    elseif idx == panelID.ActionGuide then
