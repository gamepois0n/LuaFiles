...
function Panle_Window_StableFunction_Info:buttonPos()
  ...
  for index = 0, self._value.currentFuncCount - 1 do
    local textSize = self._funcButtonList[index].radioButton:GetTextSizeX()
    if maxTextSize < textSize then
      maxTextSize = textSize
    end
  end
  self._pos.sizeFunPosX = maxTextSize
  ...
  if 0 == self._value.currentFuncCount or 1 == self._value.currentFuncCount then
    ...
    self._pos.startPosX = (sizeX - self._pos.rangeX) / 2 + self._pos.sizeFunPosX / 2
    self._pos.spaceFunPosX = self._pos.rangeX / self._value.currentFuncCount
  end
  for index = 0, self._value.currentFuncCount - 1 do
    self._funcButtonList[index].radioButton:SetPosX(self._pos.startPosX + self._pos.spaceFunPosX * index)
  end
end
