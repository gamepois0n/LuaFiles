local Panel_Window_StableMarket_info = {
  ...
  _config = {maxSlotCount = 4, maxPage = 99},
  _texture = {
...
function PaGlobalFunc_StableMarket_SelectList(index)
  ...
  if true == PaGlobalFunc_StableMarket_ChecKTabMy() then
    ...
    if nil ~= myAuctionInfo then
      ...
      self._slotMarket[self._value.currentMarketIndex].staticText_Select_ConsoleUI:SetShow(false)
      if true == self._value.openByNpc then
        self._slotMarket[self._value.currentMarketIndex].staticText_Select_ConsoleUI:SetShow(true)
        local servantInfo = stable_getServantByServantNo(auctionServantInfo:getServantNo())
        if nil ~= servantInfo then
          if CppEnums.ServantStateType.Type_RegisterMarket == servantInfo:getStateType() then
            if isAuctionEnd then
              self._slotMarket[self._value.currentMarketIndex].staticText_Select_ConsoleUI:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_STABLEMARKET_END"))
            else
              self._slotMarket[self._value.currentMarketIndex].staticText_Select_ConsoleUI:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_STABLEMARKET_BTN_CANCEL"))
            end
          else
            self._slotMarket[self._value.currentMarketIndex].staticText_Select_ConsoleUI:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_STABLEMARKET_BTN_RECEIVE"))
          end
        else
          self._slotMarket[self._value.currentMarketIndex].staticText_Select_ConsoleUI:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_STABLEMARKET_BTN_RECEIVE"))
        end
  ...
  elseif true == self._value.openByNpc then
    self._slotMarket[self._value.currentMarketIndex].staticText_Select_ConsoleUI:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_STABLEMARKET_BTN_BUY"))
    self._slotMarket[self._value.currentMarketIndex].staticText_Select_ConsoleUI:SetShow(true)
  end
