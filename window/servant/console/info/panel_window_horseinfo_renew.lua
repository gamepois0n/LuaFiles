...
function Panel_Window_HorseInfo_info:updateBaseInfo()
  ...
  if servantInfo:doMating() and 9 ~= servantInfo:getTier() then
    ...
    if servantInfo:doClearCountByMating() then
      descText = descText .. "(" .. PAGetString(Defines.StringSheet_GAME, "LUA_SERVANT_RESET_POSSIBLE") .. ")"
    else
      descText = descText .. "(" .. PAGetString(Defines.StringSheet_GAME, "LUA_SERVANT_RESET_IMPOSSIBLE") .. ")"
    end
  ...
  descText = descText .. PAGetString(Defines.StringSheet_GAME, "LUA_STABLEINFO_KILLCOUNT")
...
function PaGlobalFunc_HorseInfo_Simpletooltips(isShow, idx)
  if not isShow then
    TooltipSimple_Hide()
    return
  end
