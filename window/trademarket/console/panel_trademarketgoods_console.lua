...
function PaGlobal_TradeMarketGoods_CreateList(content, key)
  ...
  stc_Icon:ChangeTextureInfoNameAsync("Icon/" .. tradeItemWrapper:getStaticStatus():getIconPath())
  txt_ItemName:SetText(tradeItemWrapper:getStaticStatus():getName())
  ...
  txt_Origin:SetText(itemWrapper:getProductionRegion())
...
function PaGlobal_TradeMarketGoods_SellItemContinue(inputNumber, param)
  ...
  if false == isLinkedNode and 0 ~= regionInfo._waypointKey then
    ...
    if 5 == tradeType then
      ...
      local realPrice = self._realPriceList[goodsIdx]
      ...
      if limitCount < Int64toInt32(inputNumber) then
        if limitCount > 0 then
          npcShop_doSellInTradeShop(itemData.invenNum, limitCount, itemData.inventoryType, 0)
        end
    ...
    if nil ~= shopItemWrapper then
      ...
      if 5 == tradeType then
        ...
        if limitCount < Int64toInt32(s64_inventoryItemCount) then
          if limitCount > 0 then
            npcShop_doSellInTradeShop(goodsIdx, limitCount, 0, 0)
          end
          Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_TOOMANYMONEYFORSELLTRADEITEM"))
          return
        end
