...
function PetListControlCreate(control, key)
  ...
  if isUnsealPet(key) then
    ...
    if true == ToClient_IsContentsGroupOpen("378") then
      if nil ~= petComposeNo[1] then
        btnFusion:SetShow(false)
      elseif petNo_s64 ~= petComposeNo[0] and nil ~= petComposeNo[0] then
        btnFusion:SetShow(true)
      elseif nil == petComposeNo[0] and true == CheckCompose() then
        btnFusion:SetShow(true)
      end
    else
      local petComposable = 2 > math.abs(petTier - composePetTier) or 99 == petRace and petTier
      if nil ~= petComposeNo[1] then
        btnFusion:SetShow(false)
      elseif petNo_s64 ~= petComposeNo[0] and nil ~= petComposeNo[0] and (petCompose._race == petRace or true == petCompose._isJokerPetUse and 99 == petRace) and petComposable then
        btnFusion:SetShow(true)
      elseif nil == petComposeNo[0] and true == CheckCompose() and petRace <= #petRaceCount and 99 ~= petRace then
        btnFusion:SetShow(true)
      end
    end
