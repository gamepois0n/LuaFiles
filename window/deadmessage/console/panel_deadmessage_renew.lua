...
function deadMessage:perFrameUpdate(deltaTime)
  ...
  if true == self._isUseButtonAbleTime then
    ...
    if self._buttonWaitTime < 0 and nil ~= self._buttonTypeArray then
      for index = 1, #self._buttonTypeArray do
        self._buttonTypeArray[index]:SetEnable()
      end
    end
  end
...
function deadMessage:buttonPushedEvent_Observer()
  ...
  if false == ToClient_getJoinGuildBattle() and false == ToClient_getPlayNowSavageDefence() and false == ToClient_IsSelfInGuildTeamBattle() then
    ShowCommandFunc(self._resurrectionTime)
  else
    ShowCommandFunc(nil)
  end
end
...
function deadMessage:notifySiegeShowWatchPanel(isShow)
  ...
  if false == Panel_GuildBattleWatchingMode:GetShow() and true == isShow then
    ...
    GuildWatchMode._remainTime = self._tempButtonAbleTimeForGuildObserver
  end
...
function deadMessage:registEventHandler()
  ...
  Panel_DeadMessage_Renew:RegisterUpdateFunc("PaGlobalFunc_DeadMessage_PerFrameUpdate")
end
