...
function serverSelect:update()
  ...
  local leftTimeStr = converStringFromLeftDateTime(channelMoveableRemainTime)
  leftTimeStr = string.gsub(leftTimeStr, PAGetString(Defines.StringSheet_GAME, "LUA_GLOBAL_TIME_HOUR"), ":")
  leftTimeStr = string.gsub(leftTimeStr, PAGetString(Defines.StringSheet_GAME, "LUA_GLOBAL_TIME_MINUTE"), ":")
  leftTimeStr = string.gsub(leftTimeStr, PAGetString(Defines.StringSheet_GAME, "LUA_GLOBAL_TIME_SECOND"), "")
  self._ui._saticText_Time:SetText(leftTimeStr)
  ...
  for chIndex = 0, channelCount - 1 do
    ...
    if nil ~= serverData then
      ...
      local isBeingWar = serverData._isSiegeBeing and channelInfo._button_Normal:GetShow()
      ...
      channelIcon:SetShow(isBeingWar)
    end
  end
end
