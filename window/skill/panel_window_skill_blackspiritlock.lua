...
function PAGlobalFunc_BlackSkillLock(skillNo, index)
  ...
  if nil ~= skillLevelInfo then
    ...
    if isBlockSkill then
      ...
      Proc_ShowMessage_Ack(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_BLACKSPIRITLOCK_UNLOCKSKILL", "skillName", skillName))
    else
      ToClient_blockBlackSpiritSkill(skillLevelInfo._skillKey)
      Proc_ShowMessage_Ack(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_BLACKSPIRITLOCK_LOCKSKILL", "skillName", skillName))
    end
  end
...
function PAGlobalFunc_BlackSkillAllLock()
  ...
  for index = 0, num - 1 do
    ...
    if nil ~= skillWrapper then
      ...
      local skillLevelInfo = getSkillLevelInfo(skillNo)
      local skillKey = skillLevelInfo._skillKey
      if skillLevelInfo._usable and skillWrapper:isLastSkill() then
        if isCheck then
  ...
  if isCheck then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_BLACKSPIRITLOCK_ALLLOCKSKILL"))
  else
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_BLACKSPIRITLOCK_ALLUNLOCKSKILL"))
  end
end
function PaGlobal_BlackSpiritSkillLock:setSkill()
  ...
  if count > 1 then
    ...
    table.sort(self._sortByRage, sortFunc)
  end
  self._ui._list2_SkillList:getElementManager():clearKey()
  for i = 0, count - 1 do
...
function PaGlobal_BlackSpiritSkillLock:open(whereType)
  ...
  if 0 == count then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_BLACKSPIRITLOCK_IMPOSSIBLE"))
    return
  end
  if 0 == whereType then
