...
function workerTrade:createControl()
  ...
  for index = 0, self._config._skillInfoCount - 1 do
    ...
    skill._slot:SetPosX(self._config._slotPos._startX)
    skill._slot:SetPosY(self._config._slotPos._startY + (skill._slot:GetSizeY() + self._config._slotPos._gapY) * index)
  ...
  for index = startWorkerDataIndex, startWorkerDataIndex + self._config._pageSlotCount - 1 do
    ...
    if nil ~= workerWrapperLua then
      ...
      elseif workerWrapperLua:getActionPoint() ~= workerWrapperLua:getMaxActionPoint() then
        info._staticText_Buy_ConsoleUI:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_WORKERAUCTION_NOTENOUGH_ACTIONPOINT"))
        info._staticText_Buy_ConsoleUI:SetTextSpan(self._config._workerBuyTextX, self._config._workerBuyTextY - info._staticText_Buy_ConsoleUI:GetTextSizeY())
      elseif workerWrapperLua:getGrade() < 2 then
        info._staticText_Buy_ConsoleUI:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_WORKERAUCTION_NOTENOUGH_GRADE"))
        info._staticText_Buy_ConsoleUI:SetTextSpan(self._config._workerBuyTextX, self._config._workerBuyTextY - info._staticText_Buy_ConsoleUI:GetTextSizeY() + 10)
...
function FGlobal_WorkerTrade_Open()
  if Panel_Dialog_RandomWorker:GetShow() or Panel_Dialog_WorkerContract:GetShow() then
