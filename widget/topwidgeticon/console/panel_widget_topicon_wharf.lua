...
local textureByType = {
  path = "new_ui_common_forlua/window/horse/servant_icon_01.dds",
  x1 = 123,
  y1 = 123,
  x2 = 183,
  y2 = 183
...
function Panel_Widget_TopIcon_Wharf_info:checkHp(currentHp, currentMaxHp, vehicleType)
  ...
  if currentHp < self._value.beforeHp and nil ~= self._value.beforeHp then
    ...
    self._hitEffect = self._ui.static_WharfIcon_Template:AddEffect("fUI_Alarm_HorseAttack04", false, -0.1, -0.15)
    local sendMsg = {
  ...
  if remainHp <= 10 then
    ...
    if nil ~= effectName then
      self._less10Effect = self._ui.static_WharfIcon_Template:AddEffect(effectName, false, -0.1, -0.15)
    end
  end
end
