...
local PanelInteraction = {
  _ui = {
    ...
    txt_TargetDesc = UI.getChildControl(_panel, "StaticText_Desc"),
...
function PanelInteraction:show(actor)
  ...
  local actorType = ""
  self._ui.txt_TargetName:SetPosY(76)
  self._ui.txt_TargetType:SetPosY(60)
  ...
  elseif actor:get():isNpc() then
    ...
    if nil ~= npcTitle and "" ~= npcTitle then
      actorType = npcTitle
      self._ui.txt_TargetName:SetPosY(91)
    end
  ...
  self._ui.txt_TargetType:SetText(actorType)
...
function PanelInteraction:updatePerFrame(deltaTime)
  ...
  if nil == selfPlayer or selfPlayer:isDead() or Panel_Looting:GetShow() or true == Panel_Window_Exchange:GetShow() or Defines.UIMode.eUIMode_Default ~= GetUIMode() then
    self:close()
    return
  end
  local actor = interaction_getInteractable()
...
function PanelInteraction:updateDesc(indteractionType)
  ...
  if nil == interactionDesc or "" == interactionDesc then
    ...
    if nil == self._ui.txt_TargetType:GetText() or "" == self._ui.txt_TargetType:GetText() then
      self._ui.txt_TargetName:SetPosY(76)
    else
      self._ui.txt_TargetName:SetPosY(91)
    end
    ...
    self._ui.txt_TargetDesc:SetPosY(60)
