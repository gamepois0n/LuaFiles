...
local MiniGame_Timing = {
  _ui = {
    ...
    stc_Press_A = UI.getChildControl(Panel_MiniGame_Timing, "Static_Press_A"),
...
function MiniGame_Timing:init()
  ...
  self._ui.stc_Press_Effect = UI.getChildControl(self._ui.stc_Press_A, "Static_Effect")
  self._ui.stc_SpaceBar_Eff = UI.getChildControl(self._ui.stc_SpaceBar, "Static_SpaceBar_Eff")
  if _ContentsGroup_isConsolePadControl then
    self._ui.stc_Press_A:SetShow(true)
...
function Panel_Minigame_Pad_Timing_Freeze(keyType)
  local self = MiniGame_Timing
  if false == self._isGameStart then
    return
  end
  if __eQuickTimeEventPadType_A == keyType and Panel_MiniGame_Timing:IsShow() and Mingame_Panel_Timing_Value_isPressed == false then
    local minValue = self._ui.stc_Timing:GetPosY() - self._ui.progress2_Timing:GetPosY()
    local maxValue = minValue + self._ui.stc_Timing:GetSizeY()
    local checkPos = self._ui.stc_TimingProgress_Head:GetPosY() + self._ui.stc_TimingProgress_Head:GetSizeY() / 2
    playMode = 2
