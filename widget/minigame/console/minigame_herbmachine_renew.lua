...
local MiniGame_HerbMachine = {
  _ui = {
    ...
    stc_SpaceBar = UI.getChildControl(Panel_MiniGame_Timing, "Static_SpaceBar"),
    stc_PressA = UI.getChildControl(Panel_MiniGame_Timing, "Static_Press_A"),
...
function MiniGame_HerbMachine:init()
  ...
  self._ui.stc_SpaceBar_Effect = UI.getChildControl(self._ui.stc_SpaceBar, "Static_SpaceBar_Eff")
...
function MiniGame_HerbMachine:timing_update_HerbMachine(deltaTime)
  ...
  if _ContentsGroup_isConsolePadControl then
    self._ui.stc_PressA:SetShow(true)
    self._ui.stc_SpaceBar:SetShow(false)
  else
    self._ui.stc_PressA:SetShow(false)
    self._ui.stc_SpaceBar:SetShow(true)
    self._ui.stc_SpaceBar_Effect:SetPosY(percent * self._ui.progress2_Timing:GetSizeY() - self._ui.stc_SpaceBar_Effect:GetSizeY() / 2)
  end
end
...
function Panel_Minigame_HerbMachine_Start()
  ...
  if _ContentsGroup_isConsolePadControl then
    self._ui.txt_Purpose:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_XBOX1_GLOBALMANUAL_FISHING_GAUGESTOP"))
  else
  end
...
function MiniGame_HerbMachineGame_PadPress(keyType)
  if __eQuickTimeEventPadType_A == keyType then
    keyType = MGKT.MiniGameKeyType_Space
  end
