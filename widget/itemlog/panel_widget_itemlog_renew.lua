...
function ItemLog.new(index, bgControl, padding)
  local log = {
    ...
    titleControl = UI.getChildControl(bgControl, "StaticText_ItmeLog"),
    itemEnchantKey = 0,
  ...
  log.titleControl:SetTextMode(CppEnums.TextMode.eTextMode_LimitText)
...
function ItemLogManager:initialize()
  ...
  for i = 1, self._config._maxShowingLogSize do
    local control = UI.cloneControl(bgTemplate, self._panel, "StaticText_ItmeLog" .. i)
