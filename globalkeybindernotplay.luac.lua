...
-- Command line: D:\BDO_PazGameData\Unpacked\luacscript\x86\globalkeybindernotplay.luac 
...
GlobalKeyBinder_UpdateNotPlay = function(deltaTime)
  ...
  do
    if (MessageBox.isPopUp)() then
      if not GlobalKeyBinder_CheckKeyPressed(VCK.KeyCode_RETURN) then
        local pcKey = GlobalKeyBinder_CheckKeyPressed(VCK.KeyCode_SPACE)
      end
      if pcKey and not _ContentsGroup_RenewUI then
        ...
        if isPadUp(__eJoyPadInputType_A) and _ContentsGroup_RenewUI then
          (MessageBox.keyProcessEnter)()
        else
