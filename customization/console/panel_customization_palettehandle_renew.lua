...
function PaGlobalFunc_Customization_PaletteHandle_CreateEyePalette(colorTempate, colorParent, focusBox, classType, paramType, paramIndex, paramIndex2, PaletteIndex, checkBoxLeft, checkBoxRight)
  ...
  for colorIndex = 0, count - 1 do
    local tempStatic = self._colorStatic[colorIndex]
    if nil == tempStatic then
      tempStatic = UI.createControl(CppEnums.PA_UI_CONTROL_TYPE.PA_UI_CONTROL_BUTTON, colorParent, "Static_Color_" .. colorIndex)
    ...
    tempStatic:addInputEvent("Mouse_On", "PaGlobalFunc_Customization_PaletteHandle_UpdateFocusBox(" .. colorIndex .. ")")
    tempStatic:addInputEvent("Mouse_Out", "PaGlobalFunc_Customization_PaletteHandle_UpdateFocusBox(-1)")
