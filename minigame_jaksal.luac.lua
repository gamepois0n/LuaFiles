...
-- Command line: D:\BDO_PazGameData\Unpacked\luacscript\ui_data\x86\widget\minigame\minigame_jaksal.luac 
...
local ui = {_fish = (UI.getChildControl)(Panel_MiniGame_Jaksal, "Static_Fish"), _water = (UI.getChildControl)(Panel_MiniGame_Jaksal, "Static_Water"), _crossHair = (UI.getChildControl)(Panel_MiniGame_Jaksal, "Static_Crosshair"), _jaksal = (UI.getChildControl)(Panel_MiniGame_Jaksal, "Static_Jaksal"), _gaugeFishPower = (UI.getChildControl)(Panel_MiniGame_Jaksal, "Progress2_FishGauge"), _gaugeJaksalPower = (UI.getChildControl)(Panel_MiniGame_Jaksal, "CircularProgress_JaksalGauge"), _result_Success = (UI.getChildControl)(Panel_MiniGame_Jaksal, "Static_Result_Success"), _result_Failed = (UI.getChildControl)(Panel_MiniGame_Jaksal, "Static_Result_Failed"), _fishGet = (UI.getChildControl)(Panel_MiniGame_Jaksal, "Static_FishGet"), _jaksalCount = (UI.getChildControl)(Panel_MiniGame_Jaksal, "StaticText_JaksalCount"), _endTimer = (UI.getChildControl)(Panel_MiniGame_Jaksal, "StaticText_EndTimer")}
;
(ui._water):setGlassBackground(true)
local fishMoveData = {position = float2(0, 0), rotate = 0, destPosition = float2(300, 0), escapePower = 1, escapePowerDecreasePerSec = 0.9, escapePowerIncreasePerSec = 0.1, fishRePositionTime = 0}
local enumJaksal = {wait = 0, powerUp = 1, fullPower = 2, powerDown = 3, shot = 4}
local sumTime = 0
...
Panel_MiniGame_Jaksal_Start = function()
  -- function num : 0_0 , upvalues : ui, fishMoveData, jaksalCount, isStoppedGame, jaksalFailCheck
...
Panel_MiniGame_Jaksal_End = function()
  -- function num : 0_1 , upvalues : jaksalCount, sumTime, currentPercent, resetTimer, jaksalShotWaitTime, fishMoveData, isEndTimer, ui, isStoppedGame, jaksalFailCheck
  jaksalCount = 15
  sumTime = 0
  currentPercent = 0
  resetTimer = 0
...
local normalize = function(float2Value)
  -- function num : 0_4 , upvalues : getLength
...
local changeDestPosition = function(useImprovedEscape, updateForce)
  -- function num : 0_5 , upvalues : ui, fishMoveData, getLength, normalize
...
local rotatePerFrame = function(startPosition, endPosition, deltaTime, useImprovedEscape, fishToDestLength)
  -- function num : 0_6 , upvalues : fishMoveData
...
local updateCrossHair = function()
  ...
  if posX < (ui._water):GetPosX() then
    ...
    if (ui._water):GetPosX() + (ui._water):GetSizeX() - (ui._crossHair):GetSizeX() < posX then
  ...
  if posY < (ui._water):GetPosY() then
    ...
    if (ui._water):GetPosY() + (ui._water):GetSizeY() - (ui._crossHair):GetSizeY() < posY then
...
local updateEscapePower = function(fishToDestLength, deltaTime)
  -- function num : 0_8 , upvalues : ui, fishMoveData
  (ui._gaugeFishPower):SetProgressRate((1 - (1 - fishMoveData.escapePower) * 2) * 100)
end

Panel_MiniGame_PerFrameUpdate = function(deltaTime)
  -- function num : 0_9 , upvalues : updateCrossHair, fishMoveData, isStoppedGame, isEndTimer, ui, changeDestPosition, updateEscapePower, rotatePerFrame, isCatched, rotateToDirection, jaksalCount, jaksalFailCheck
  ...
  if isStoppedGame == false then
    ...
    if (math.floor)(isEndTimer) <= 50 then
  ...
  if ((math.floor)(isEndTimer) <= 0 or jaksalCount < 0) and jaksalFailCheck == true then
    Panel_MiniGame_Jaksal_Failed()
    jaksalFailCheck = false
  end
end

Panel_MiniGame_FillGaugeUpdate = function(deltaTime, shotSuccess)
  -- function num : 0_10 , upvalues : sumTime, ui
...
Panel_MiniGame_Jaksal_KeyPressCheck = function(keyType)
  -- function num : 0_11 , upvalues : ui, MGKT, sumTime, isStoppedGame, jaksalCount, fishMoveData, getLength
...
Panel_MiniGame_Jaksal_Success = function()
  -- function num : 0_12 , upvalues : ui, isEndTimer, isStoppedGame
...
Panel_MiniGame_Jaksal_Failed = function()
  -- function num : 0_13 , upvalues : ui, isStoppedGame
