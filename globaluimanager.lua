...
function close_WindowPanelList()
  ...
  if false == _ContentsGroup_RenewUI_ReinforceSkill and Panel_SkillReinforce:GetShow() then
  ...
  if false == _ContentsGroup_RenewUI and Panel_Window_GameTips:IsShow() then
  ...
  for i = 1, #panel_WindowList do
    if nil ~= panel_WindowList[i] and panel_WindowList[i]:IsUISubApp() == false then
      panel_WindowList[i]:SetShow(false, false)
    end
  end
  for i = 1, #panel_HideListAnimationTrue do
    if nil ~= panel_HideListAnimationTrue[i] and panel_HideListAnimationTrue[i]:IsUISubApp() == false then
      panel_HideListAnimationTrue[i]:SetShow(false, true)
    end
  end
  for i = 1, #panel_SoundedWindowList do
    if nil ~= panel_SoundedWindowList[i] and panel_SoundedWindowList[i]:IsUISubApp() == false and false == _ContentsGroup_RenewUI and 1 ~= i then
      panel_SoundedWindowList[i]:SetShow(false, false)
    end
  end
  if true == _ContentsGroup_RenewUI_Dailog then
    if not PaGlobalFunc_MainDialog_GetShow() then
      for _, loopPanel in ipairs(panel_ExceptionWindowList) do
        if nil ~= loopPanel and loopPanel:IsUISubApp() == false then
...
function close_force_WindowPanelList()
  ...
  if false == _ContentsGroup_RenewUI then
    if true == Panel_Window_Quest_New:GetShow() then
  ...
  for i = 1, #panel_SoundedWindowList do
    if nil ~= panel_SoundedWindowList[i] and panel_SoundedWindowList[i]:IsUISubApp() == false then
      panel_SoundedWindowList[i]:SetShow(false, false)
    end
  end
  if true == _ContentsGroup_RenewUI_Dailog then
